...
# ==========================================
# 0. 自動檢查 / 安裝缺失套件 (三重突破機制)
# ==========================================
_REQUIRED_PACKAGES = ['requests', 'pandas', 'openpyxl', 'tqdm', 'urllib3']

def _ensure_packages():
    missing = [p for p in _REQUIRED_PACKAGES if importlib.util.find_spec(p) is None]
    if not missing:
        return
    print("🔧 偵測到缺少套件，嘗試自動安裝: " + ", ".join(missing))
    import subprocess
    strategies = [
    ...
    for base in strategies:
        try:
            r = subprocess.run(base + missing, capture_output=True, text=True, timeout=300)
            if r.returncode == 0:
                for p in missing:
                    ...
                    if importlib.util.find_spec(p) is not None:
                        ...
                        print(f"⚠️ {p} 安裝後仍未偵測到 (可能需重啟程式)")
                return
            print(f"⚠️ 策略失敗 ({base[0]}): {r.stderr.strip()[:200]}")
        except Exception as e:
            print(f"⚠️ 安裝策略異常: {e}")
    print("❌ 套件安裝失敗，請手動執行: pip install " + " ".join(missing))
...
# ==========================================
# 0.5 優雅中斷 (Graceful Shutdown) — 攔截 Ctrl+C / SIGTERM
# ==========================================
graceful_stop = False
...
# ==========================================
# 0.6 執行時間守門員 (GitHub Actions 專用)
#     設 RUN_LIMIT_MINUTES=330 → 5.5 小時後自動觸發優雅中斷並存檔。
#     本機執行不設此環境變數，則不啟動、無時間限制。
# ==========================================
def _start_watchdog():
    limit_env = os.environ.get("RUN_LIMIT_MINUTES", "").strip()
    if not limit_env:
        return
    try:
        limit_min = float(limit_env)
    except ValueError:
        print(f"⚠️ RUN_LIMIT_MINUTES 無效 ({limit_env})，忽略時間限制")
        return
    ...
    def _watch():
        ...
        while not graceful_stop:
            ...
            if elapsed_min >= limit_min:
                print(f"\n⏰ 已執行約 {elapsed_min:.0f} 分鐘，"
                      f"逼近 GitHub 6 小時上限，開始優雅中斷並儲存進度...")
                graceful_stop = True
                return
            time.sleep(5)

    threading.Thread(target=_watch, daemon=True).start()
    print(f"🛰️  GitHub Actions 模式啟動 (每輪上限 {limit_min} 分鐘)")
...
def check_7_day_cycle():
    ...
    if time.time() - prog.get("cycle_start", 0) > 604800:
        print("\n⏳ [週期檢查] 超過 7 天，啟動全面掃描模式（更新所有最新年份資料）...")
...
def main_scraper_all():
    ...
    folder_path = os.path.join(SCRIPT_DIR, sanitize_filename("胎壓資料庫_每週更新版"))
